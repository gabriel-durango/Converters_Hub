...
export default function Pdf() {
  useSEO({
    title: "PDF a imágenes (cola) | Converters Hub",
    description: "Convierte PDF a imágenes optimizadas mediante jobs asíncronos con progreso.",
  });
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/jobs/pdf-to-image`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fileUrl: uploadedFileUrl }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Error al iniciar la conversión del PDF");
      setJobId(data.jobId);
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  return (
    <div>
      <div className="hero">
        ...
        <p>Convierte páginas de PDF a imágenes optimizadas. El procesamiento se realiza en segundo plano.</p>
      </div>
      <div className="card">
        <FileDropzone
          onFile={handleFile}
          ...
          label="Arrastra un PDF o haz clic para cargar"
        />
        ...
        >
          {loading ? "Iniciando conversión..." : "Convertir a imágenes"}
        </button>

        {error && <p className="error" style={{ marginTop: "1rem" }}>{error}</p>}
      </div>
      <JobStatus queue="pdf" jobId={jobId} />
    </div>
  );
}
