...
export default function Docx() {
  useSEO({
    title: "DOCX a PDF (cola) | Converters Hub",
    description: "Convierte DOCX a PDF encolando trabajos y consulta su progreso.",
  });
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/jobs/docx-to-pdf`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fileUrl: uploadedFileUrl }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Error al iniciar la conversión del DOCX");
      setJobId(data.jobId);
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  return (
    <div>
      <div className="hero">
        <h1>DOCX a PDF</h1>
        <p>Convierte documentos DOCX a PDF. El procesamiento se realiza en segundo plano sin bloquear tu navegador.</p>
      </div>
      <div className="card">
        <FileDropzone
          onFile={handleFile}
          ...
          label="Arrastra un DOCX o haz clic para cargar"
        />
        ...
        >
          {loading ? "Iniciando conversión..." : "Convertir a PDF"}
        </button>
